...
def build_manager_patch_payload(manager_scim_id: str) -> Dict[str, Any]:
    """Build a PATCH payload that assigns an already provisioned SCIM user as manager."""
    return {
        "schemas": ["urn:ietf:params:scim:api:messages:2.0:PatchOp"],
        "Operations": [
            {
...
def main() -> int:
    ...
    updated_count = 0
    failure_count = 0
    manager_success_count = 0
    manager_missing_count = 0
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        mapped = {
            ...
            "Supervisor": get_first_value(row, ["Supervisor"]),
            "SupervisorEmployeeId": get_first_value(row, ["Supervisor Employee ID"]),
            "Locale": "en-US",
        }
    ...
    # Phase 2: assign managers only after all users in the CSV have been created/updated.
    # Supervisor Employee ID contains the supervisor's Interact username (for example BR568).
    # Resolve that username to the supervisor's SCIM id, then assign the manager relationship.
    print()
    print("=== Assigning Managers ===")

    for row in rows:
        user_name = get_first_value(row, ["Username", "Email Address"])
        supervisor_name = get_first_value(row, ["Supervisor"])
        supervisor_employee_id = get_first_value(row, ["Supervisor Employee ID"])

        if not supervisor_employee_id:
            if supervisor_name and args.show_progress:
                print(
                    f"Skipping manager assignment for {user_name or '<missing username>'}: "
                    f"Supervisor is '{supervisor_name}' but Supervisor Employee ID is blank"
                )
            continue

        if not user_name:
            print(
                f"Cannot assign supervisor {supervisor_employee_id}: employee Username is missing"
            )
            manager_failure_count += 1
            continue

        if supervisor_employee_id == user_name:
            print(
                f"Cannot assign manager for {user_name}: Supervisor Employee ID matches the employee's own Username"
            )
            manager_failure_count += 1
            continue

        try:
            employee_exists, employee_scim_id = test_user_exists(
                user_name, args.bearer_token, args.base_url
            )
            if not employee_exists or not employee_scim_id:
                print(f"Cannot assign manager: employee not found in Interact: {user_name}")
                manager_failure_count += 1
                continue

            manager_exists, manager_scim_id = test_user_exists(
                supervisor_employee_id, args.bearer_token, args.base_url
            )
            if not manager_exists or not manager_scim_id:
                label = f" ({supervisor_name})" if supervisor_name else ""
                print(
                    f"Manager not found in Interact: {supervisor_employee_id}{label} "
                    f"for employee {user_name}"
                )
                manager_missing_count += 1
                continue

            payload = build_manager_patch_payload(manager_scim_id)
            request_json(
                "PATCH",
                f"{args.base_url.rstrip('/')}/{employee_scim_id}",
                args.bearer_token,
                payload,
            )
            manager_success_count += 1

            if args.show_progress:
                label = f" ({supervisor_name})" if supervisor_name else ""
                print(
                    f"Manager assigned: {user_name} -> "
                    f"{supervisor_employee_id}{label}"
                )

        except Exception as exc:
            manager_failure_count += 1
            print(
                f"Failed manager assignment for {user_name} -> "
                f"{supervisor_employee_id}: {exc}"
            )
    ...
    print(f"Managers assigned: {manager_success_count}")
    print(f"Managers not found: {manager_missing_count}")
    print(f"Manager assignment failures: {manager_failure_count}")
...
Location, Company, Title, Start Date, End Date, Supervisor, Supervisor Employee ID

*** Manager assignment runs in a second pass after user creation/update.
Supervisor Employee ID must contain the supervisor's Interact Username (for example BR568).
The Supervisor column is optional for SCIM and is used only to improve validation/log output.
